...
public class Commands extends ConfigData implements CommandExecutor, TabCompleter {
    ...
    private void openMainMenu(Player staff) {
        ...
        InventoryRollback.getInstance().getScheduler().runTaskAsynchronously(menu::getMainMenu);
    }
    ...
    private void openPlayerMenu(Player staff, OfflinePlayer offlinePlayer) {
        ...
        staff.openInventory(menu.getInventory());
        InventoryRollback.getInstance().getScheduler().runTaskAsynchronously(menu::getPlayerMenu);
    }
    ...
    private void convertMySQL(CommandSender sender) {
        if (sender instanceof ConsoleCommandSender && sender.isOp()) {
            InventoryRollback.getInstance().getScheduler().runTaskAsynchronously(MySQL::convertYAMLToMySQL);
        }
    }

    private void convertYAML(CommandSender sender) {
        if (sender instanceof ConsoleCommandSender && sender.isOp()) {
            InventoryRollback.getInstance().getScheduler().runTaskAsynchronously(YAML::convertOldBackupData);
        }
    }
